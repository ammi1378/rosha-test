...
import {
  ...
  IServiceFilters,
  IVipFilters,
  VipServiceFilterProps,
} from "./VipServiceFilter.props";
...
const VipServiceFilterComponent: React.FC<VipServiceFilterProps> = ({
  className,
  service,
  serviceInfo,
}) => {
  const router = useRouter();
  const [filtersValue, setFiltersValue] = useState<IVipFilters>({
    keyword: "",
    rating: "",
    city: "",
    VipService: "",
  ...
  useEffect(() => {
    ...
    const VipServiceFilter = parsedQueryString?.filters?.Card?.VipService?.$in as
      | string[]
      | undefined;

    if (VipServiceFilter && localServicesFilters.filters.VipService) {
      localServicesFilters.filters.VipService.value = VipServiceFilter.map(
        (val) => val
      );
    }
  ...
  useEffect(() => {
    ...
    if (servicesFilters.filters.VipService?.value.length) {
      filters.Card.VipService = {
        $in: servicesFilters.filters.VipService?.value,
      };
    }
  ...
  const serviceFilterChange = (filter: IServiceFilter, value: any) => {
    const localServicesFilters = { ...servicesFilters };
    if (filter.type === "season" && localServicesFilters.filters.VipService) {
      localServicesFilters.filters.VipService.value = value;
    }
              ...
              return (
                <li className="form-check form-check-block" key={food}>
                  <input
                    ...
                    onChange={() =>
                      changeServiceRate(
                        servicesFilters.filters.VipService!,
                        food
                      )
                    }
                    checked={servicesFilters.filters.VipService?.value?.includes(
                      food
                    )}
                  />
...
export default VipServiceFilterComponent;
