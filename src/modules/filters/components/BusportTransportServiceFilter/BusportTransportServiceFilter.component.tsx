...
import {
  IServiceFilter,
  IServiceFilters,
  IBusportTransportFilters,
  BusportTransportServiceFilterProps,
} from "./BusportTransportServiceFilter.props";

const tourServiceFilterComponent = "tourServiceFilterComponent";
const ratings = [5, 4, 3, 2, 1];


const BusportTransportServiceFilterComponent: React.FC<BusportTransportServiceFilterProps> = ({
  className,
  service,
  serviceInfo,
}) => {
  const router = useRouter();
  const [filtersValue, setFiltersValue] = useState<IBusportTransportFilters>({
    keyword: "",
    rating: "",
    max: 0,
    min: 0,
  ...
  useEffect(() => {
    ...
    const filters: IBusportTransportFilters = {
      keyword: "",
      rating: "",
      min: 0,
      max: 0,
    ...
    const minFilter = parsedQueryString?.filters?.Card?.Min?.$in as
      | number
      | undefined;
    ...
    const maxFilter = parsedQueryString?.filters?.Card?.Max?.$in as
      | number
      | undefined;
...
export default BusportTransportServiceFilterComponent;
