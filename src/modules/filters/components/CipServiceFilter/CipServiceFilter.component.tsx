...
import {
  ...
  IServiceFilters,
  ICipFilters,
  CipServiceFilterProps,
} from "./CipServiceFilter.props";

const resturantServiceFilterComponent = "resturantServiceFilterComponent";
const ratings = [5, 4, 3, 2, 1];
const vipServices = ["Complimentary Buffet", "Free WIFI", "Personal Driver to Aircraft", "No Check-In Lines", "No Passport Control Lines"];
const cities = ["Tehran", "Kish", "Mashhad", "Esfehan"];

const CipServiceFilterComponent: React.FC<CipServiceFilterProps> = ({
  className,
  service,
  serviceInfo,
}) => {
  const router = useRouter();
  const [filtersValue, setFiltersValue] = useState<ICipFilters>({
    keyword: "",
    rating: "",
    city: "",
    CipService: "",
  ...
  useEffect(() => {
    ...
    const CipServiceFilter = parsedQueryString?.filters?.Card?.CipService?.$in as
      | string[]
      | undefined;

    if (CipServiceFilter && localServicesFilters.filters.CipService) {
      localServicesFilters.filters.CipService.value = CipServiceFilter.map(
        (val) => val
      );
    }
  ...
  useEffect(() => {
    ...
    if (servicesFilters.filters.CipService?.value.length) {
      filters.Card.CipService = {
        $in: servicesFilters.filters.CipService?.value,
      };
    }
  ...
  const serviceFilterChange = (filter: IServiceFilter, value: any) => {
    const localServicesFilters = { ...servicesFilters };
    if (filter.type === "season" && localServicesFilters.filters.CipService) {
      localServicesFilters.filters.CipService.value = value;
    }
              ...
              return (
                <li className="form-check form-check-block" key={food}>
                  <input
                    ...
                    onChange={() =>
                      changeServiceRate(
                        servicesFilters.filters.CipService!,
                        food
                      )
                    }
                    checked={servicesFilters.filters.CipService?.value?.includes(
                      food
                    )}
                  />
...
export default CipServiceFilterComponent;
